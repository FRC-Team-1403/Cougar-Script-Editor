...
public class Point {
	...
	public static Point fromPolar(double distance, double angle) {
		double cx = distance * Math.cos(Math.toRadians(angle));
	...
	public Point toFeet() {
		...
		return new Point((x - img.x - img.w/2.0)/scale, (img.y - y + img.h/2.0)/scale);
	}
	...
	public Point toPixels() {
		...
		Button img = mw.imageRect;
		double scale = mw.getFieldImageScale();
		return new Point((x + (img.w/2 / scale) + (img.x / scale))*scale, ((img.y / scale) - y + (img.h/2 / scale))*scale);
	}
